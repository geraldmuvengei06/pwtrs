...
<template>
    <div class="min-w-full min-h-screen bg-white">
        <div
            ...
            <img class="p-2 w-full sm:w-1/2 " v-if="error?.statusCode == 404" src="/img/404 Error-pana.png" />
            <img class="p-2 w-full sm:w-1/2 " v-if="error?.statusCode == 500"
                src="/img/500 Internal Server Error-pana.png" />
            <img class="p-2 w-full sm:w-1/2 " v-else-if="![404, 500].includes(error?.statusCode)" src="/img/504 Error Gateway Timeout-pana.png" />
        </div>
